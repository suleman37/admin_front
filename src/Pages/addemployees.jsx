...
const EmployeeCards = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        toast.error(
          `Error fetching employees: ${error.response?.data || error.message}`
        );
      }
    };
  ...
  const handleDeleteEmployee = async (employeeId) => {
    try {
      await axios.delete(`http://16.170.204.4:5000/api/employees/${employeeId}`);
      setEmployees(employees.filter((employee) => employee._id !== employeeId));
      toast.success("Employee deleted successfully.");
    } catch (error) {
      toast.error(
        `Error deleting employee: ${error.response?.data || error.message}`
      );
    }
  };
